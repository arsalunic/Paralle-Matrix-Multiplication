...
void MultA(int* c, int* a, int* b, unsigned int n){
	...
	if(n < 4){
		...
		*c += *a * *b;
		...
		MultA(c, a, b, n / 4);
		MultA(c + (n / 4), a, b + (n / 4), n / 4);
		MultA(c + ((n / 4) * 3), a + ((n / 4) * 2), b + (n / 4), n / 4);
		MultA(c + ((n / 4) * 2), a + ((n / 4) * 2), b, n / 4);
		
		
		MultA(c + ((n / 4) * 2), a + ((n / 4) * 3), b + ((n / 4) * 2), n / 4);
		MultA(c + ((n / 4) * 3), a + ((n / 4) * 3), b + ((n / 4) * 3), n / 4);
		MultA(c + (n / 4), a + (n / 4), b + ((n / 4) * 3), n / 4);
		MultA(c, a + (n / 4), b + ((n / 4) * 2), n / 4);
